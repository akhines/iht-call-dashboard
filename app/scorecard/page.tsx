...
// Mirror of ACTIVE_SELLER_PIPELINES in builder.ts. Used to render pipeline
// chip labels in the drilldown.
...
const GHL_LOCATION_ID = "Js1OZngrzWeqSwiHjbRx";
...
// Leads drilldown — the pipeline-rewrite "v2" modal. Shown when Ashley clicks
// a Leads cell. Renders:
//   • total count
//   • pipeline chips row (counts per whitelisted pipeline)
//   • channel chips row (counts per source)
//   • scrollable list (Name → GHL contact page, Pipeline, Channel, Added ET)
function LeadsDrilldownModal({
  title,
  byPipeline,
  byChannel,
  leads,
...
// Drilldown modal (source breakdown — appts, A-B, settled — unchanged)
...
export default function ScorecardPage() {
  ...
  function renderCell(row: Record<string, number | string>, col: Column, isSummary = false, weekIdx?: number) {
    ...
    if (!isSummary && weekIdx !== undefined && num > 0) {
      ...
      if (col.key === "leads" && (week?.leadDetails || week?.leadsBySource)) {
        // New v2 drilldown when builder has populated leadDetails (pipeline-rewrite).
        // Falls back to source-only modal for legacy KV entries.
        if (week.leadDetails && week.leadDetails.length > 0) {
          return (
            <button
              onClick={() =>
                setLeadsDrilldown({
                  title: `Leads — ${fmtDate(week.startDate)}`,
                  byPipeline: week.leadsByPipeline || {},
                  byChannel: week.leadsByChannel || week.leadsBySource || {},
                  leads: week.leadDetails || [],
                })
              }
              className="text-blue-600 hover:text-blue-800 underline font-medium"
            >
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      {leadsDrilldown && (
        <LeadsDrilldownModal
          ...
          byPipeline={leadsDrilldown.byPipeline}
          byChannel={leadsDrilldown.byChannel}
          leads={leadsDrilldown.leads}
