...
// GET /api/scorecard/refresh/restore?backup=<ISO timestamp>&secret=<CRON_SECRET>
//
// Copies a snapshot blob (written by /api/scorecard/refresh?all=true before
// rebuild) back to the per-week keys. Use when a ?all=true rebuild returned
// truncated data and overwrote good per-week entries.
//
// Auth: Authorization: Bearer <CRON_SECRET> OR ?secret=<CRON_SECRET>.
//
// Modes:
//   ?backup=<timestamp>           — restore from the named blob
//   ?backup=<timestamp>&list=true — preview the blob's contents (no write)
export async function GET(request: Request) {
  ...
  // Accept the timestamp with or without the prefix.
  ...
  try {
    ...
    console.log(
      `Scorecard restore: restored ${writes.filter((w) => w.persisted).length}/${weeks.length} weeks from ${fullKey}`
    );
    ...
    console.error("Scorecard restore error:", error);
    return NextResponse.json(
